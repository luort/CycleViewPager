...
public class CycleViewPager extends FrameLayout
    ...
    private List<ImageInfo> mImageInfos;//数据集合
     ...
     */
    public void setData(List<ImageInfo> list, ImageCycleViewListener listener, int showPosition) {
        ...
        if (isCycle) {
            //添加轮播图View，数量为集合数+2
            // 将最后一个View添加进来
            mViews.add(getImageView(mContext, mImageInfos.get(mImageInfos.size() - 1).getImgUrl()));
            for (int i = 0; i < mImageInfos.size(); i++) {
                mViews.add(getImageView(mContext, mImageInfos.get(i).getImgUrl()));
            }
            // 将第一个View添加进来
            mViews.add(getImageView(mContext, mImageInfos.get(0).getImgUrl()));
        } else {
            //只添加对应数量的View
            for (int i = 0; i < mImageInfos.size(); i++) {
                mViews.add(getImageView(mContext, mImageInfos.get(i).getImgUrl()));
...
//        backGround.setBackgroundResource(R.color.cycle_image_bg);
   ...
   /* private void setIndicator(int selectedPosition) {
        setText(mTitle, mImageInfos.get(selectedPosition).getDescription());
        try {
    ...
    public void setData(List<ImageView> views, List<ImageInfo> list, ImageCycleViewListener listener) {
     ...
     *            默认显示位置
     */
    public void setData(List<ImageView> views, List<ImageInfo> list, ImageCycleViewListener listener, int showPosition) {
        ...
        mIndicatorLayout.removeAllViews();
        for (int i = 0; i < mIndicators.length; i++) {
            View view = LayoutInflater.from(mContext).inflate(R.layout.layout_cycle_viewpager_indicator, null);
    ...
    private void setIndicator(int selectedPosition) {
        setText(mTitle, mImageInfos.get(selectedPosition).getDescription());
        for (int i = 0; i < mIndicators.length; i++) {
            mIndicators[i].setBackgroundResource(R.mipmap.ad_point_nor);
        }
        if (mIndicators.length > selectedPosition)
            mIndicators[selectedPosition].setBackgroundResource(R.mipmap.ad_point_sel);
    }
            ...
            if (mImageCycleViewListener != null) {
                v.setOnClickListener(new OnClickListener() {
                    ...
                    public void onClick(View v) {
                        mImageCycleViewListener.onImageClick(mImageInfos.get(mCurrentPosition - 1), mCurrentPosition, v);
                    }
                });
            }
            container.addView(v);
